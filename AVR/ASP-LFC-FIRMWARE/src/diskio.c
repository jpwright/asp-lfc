...
#define CS_LOW()	PORTC.OUT &= ~_BV(4)			/* CS=low */
#define	CS_HIGH()	PORTC.OUT |=  _BV(4)			/* CS=high */
#define SOCKINS		(!(PORTC.IN & _BV(0)))	/* Card detected.   yes:true, no:false, default:true */
#define SOCKWP		(PORTC.IN & _BV(1))		/* Write protected. yes:true, no:false, default:false */
//#define	FCLK_SLOW()	SPCR = 0x52		/* Set slow clock (F_CPU / 64) */
//#define	FCLK_FAST()	SPCR = 0x50		/* Set fast clock (F_CPU / 2) */
//#define FCLK_SLOW()  SPIC_CTRL |= _BV(7);
//#define FCLK_FAST()  SPIC_CTRL &= ~_BV(7);
#define FCLK_SLOW() SPIC_CTRL |= _BV(7); SPIC_CTRL |= 0b00000011
#define FCLK_FAST() SPIC_CTRL |= _BV(7); SPIC_CTRL &= ~0b00000011
...
void xmit_spi(BYTE d)
{
	PORTC.OUT &= ~(1<<4);  // lower ss line indicating start of transfer
	SPIC.DATA = d;      // initiate write
	// wait for transfer complete
	while(!(SPIC.STATUS & (1<<7)));
	PORTC.OUT |= (1<<4);   // raise ss line indicating end of transfer
};
